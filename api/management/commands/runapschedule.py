...
def cleanUp():
    ...
    for dir in os.listdir(CHUNKED_UPLOAD_PATH_BASE):
        if dir != currentYear:
            try:
                os.chmod(os.path.join(CHUNKED_UPLOAD_PATH_BASE, dir), 777)
    ...
    for dir in os.listdir(os.path.join(CHUNKED_UPLOAD_PATH_BASE, currentYear)):
        if dir != currentMonth:
            try:
                os.chmod(os.path.join(os.path.join(os.path.join(CHUNKED_UPLOAD_PATH_BASE, currentYear)), dir), 777)
    ...
    for dir in os.listdir(os.path.join(os.path.join(CHUNKED_UPLOAD_PATH_BASE, currentYear), currentMonth)):
        if dir != currentDay:
            try:
                os.chmod(os.path.join(os.path.join(os.path.join(CHUNKED_UPLOAD_PATH_BASE, currentYear), currentMonth), dir), 777)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            cleanUp,
            trigger=CronTrigger(hour=13, minute=58),  # Every 10 seconds
