...
function calculateDataFrameValues(sliderOneValue, sliderTwoValue, enzymeData, restrictionEnzymes, currentSelectedFragmentSize, experimentalAdaptorContamination, experimentalOverlaps){
   ...
   sumAllBasesOfEveryBin = Math.round( sliderOneValue >= basepairLengthToBeSequenced/10 ? sumAllBasesOfEveryBin + experimentalOverlaps*parseInt(basepairLengthToBeSequenced) + experimentalAdaptorContamination*(2/3)*parseInt(basepairLengthToBeSequenced) : sumAllBasesOfEveryBin + experimentalAdaptorContamination*parseInt(basepairLengthToBeSequenced))
